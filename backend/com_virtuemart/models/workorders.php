 ...
 * @package VirtueMart
 */
class VirtueMartModelWorkrders extends VmModel {
...
// 			}
if ($data->order_status != 'P' AND (JRequest::getVar('orders_'.$data->virtuemart_order_id.'_customer_notified')==1 OR JRequest::getVar('customer_notified')==1)){
			$this->notifyCustomer( $data->virtuemart_order_id , $inputOrder );
			}
			$this->notifyCustomeradmn( $data->virtuemart_order_id , $inputOrder );
			//$zameh=JRequest::getvar( 'orders_59_verification_notified' );
			//vmInfo( JText::_($string,false).' sameh ' . $zameh);
			//print_r(JRequest::get( 'post' ));
			// send to customer
			//$vars['includeComments'] = JRequest::getVar('customer_notified', array());
			
			$whonoted='';
			if (JRequest::getVar('verification_notified')==1 or JRequest::getVar('orders_'.$data->virtuemart_order_id.'_verification_notified')==1){
			$this->notifyCustomerver( $data->virtuemart_order_id , $inputOrder );
			$whonoted .='Verification Notified <br>';
			}
			if (JRequest::getVar('agent_notified')==1 or JRequest::getVar('orders_'.$data->virtuemart_order_id.'_agent_notified')==1){
			$this->notifyCustomerag( $data->virtuemart_order_id , $inputOrder );
			$whonoted .='Agent Notified <br>';
			}
			if (JRequest::getVar('shipping_notified')==1 or JRequest::getVar('orders_'.$data->virtuemart_order_id.'_shipping_notified')==1){
			$this->notifyCustomersh( $data->virtuemart_order_id , $inputOrder );
			$whonoted .='Shipping Notified <br>';
			}
			if (JRequest::getVar('art_notified')==1 or JRequest::getVar('orders_'.$data->virtuemart_order_id.'_art_notified')==1){
			$this->notifyCustomerart( $data->virtuemart_order_id , $inputOrder );
			$whonoted .='ART Notified <br>';
			}
			//new work
			if (JRequest::getVar('fax_notified')==1 or JRequest::getVar('orders_'.$data->virtuemart_order_id.'_fax_notified')==1){
			$this->notifyCustomerfax( $data->virtuemart_order_id , $inputOrder );
			$whonoted .='FAX Notified <br>';
			}

			if (JRequest::getVar('engraver_notified')==1 or JRequest::getVar('orders_'.$data->virtuemart_order_id.'_engraver_notified')==1){
			$this->notifyCustomerengraver( $data->virtuemart_order_id , $inputOrder );
			$whonoted .='Engraver Notified <br>';
			}
			
			if (JRequest::getVar('production_notified')==1 or JRequest::getVar('orders_'.$data->virtuemart_order_id.'_production_notified')==1){
			$this->notifyCustomerproduction( $data->virtuemart_order_id , $inputOrder );
			$whonoted .='Production Notified <br>';
			}
			
			if (JRequest::getVar('leader_notified')==1 or JRequest::getVar('orders_'.$data->virtuemart_order_id.'_leader_notified')==1){
			$this->notifyCustomerleader( $data->virtuemart_order_id , $inputOrder );
			$whonoted .='Leader Notified <br>';
			}
			if (JRequest::getVar('other_email')!=''){
			$othermail=JRequest::getVar('other_email');
			$this->notifyCustomerother( $data->virtuemart_order_id , $inputOrder, $othermail);
			$whonoted .=$othermail.' Notified <br>';
			}
			
			if (JRequest::getVar('oadmin_notified')==1 or JRequest::getVar('orders_'.$data->virtuemart_order_id.'_oadmin_notified')==1){
			$this->notifyCustomeoadmin( $data->virtuemart_order_id , $inputOrder );
			$whonoted .='Admin Notified <br>';
			}
			
			$inputOrder['comments'] = trim($inputOrder['comments']);
			$comnt=$inputOrder['comments'].'-'.$whonoted;
			/* Update the order history */
			$this->_updateOrderHist($virtuemart_order_id, $data->order_status, $inputOrder['customer_notified'], $comnt);
...
*/
//sameh work
	  private function notifyCustomersadmn($virtuemart_order_id, $newOrderData = 0 ) {
		...
		// Send the email
$db =& JFactory::getDBO();
		$sql = "SELECT `email` FROM #__agentbase_agents where `name`='#notify_sales_admin'"; 
		$db->setQuery($sql);
		$artzzz = $db->loadresult();
		
		shopFunctionsF::renderMail('notf', $artzzz, $vars, null,$vars['doVendor2'],$this->useDefaultEmailOrderStatus);
		...
		if($res!=-1){
			vmInfo( vmText::_($string,false).' sales_admin '.$artzzz);
		}
		...
		if($app->isSite()){
			if (!class_exists('VirtueMartCart'))
				require(VMPATH_SITE . DS . 'helpers' . DS . 'cart.php');
			$cart = VirtueMartCart::getCart();
			$cart->customer_notified = false;
		}
		return true;
	}

	
	 private function notifyCustomeradmn($virtuemart_order_id, $newOrderData = 0 ) {
...
if (isset($newOrderData['customer_notified']) && $newOrderData['customer_notified']==0) {
		...
		// Send the email
	//	shopFunctionsF::renderMail('notf', 'someone@somewhere.com', $vars, null,$vars['doVendor'])
		$res=shopFunctionsF::renderMail('notf', 'orderhistory@trilliumorderdesk.com', $vars, null,$vars['doVendor'],$this->useDefaultEmailOrderStatus);

		if(is_object($res) or !$res){
			$string = 'COM_VIRTUEMART_NOTIFY_CUSTOMER_ERR_SEND';
			vmdebug('notifyCustomer function shopFunctionsF::renderMail throws JException');
			$res = 0;
			vmTrace('notifyCustomer1');
		} //We need this, to prevent that a false alert is thrown.
		else if ($res and $res!=-1) {
			$string = 'COM_VIRTUEMART_NOTIFY_CUSTOMER_SEND_MSG';
			vmTrace('notifyCustomer2');
		}

		if($res!=-1){
			vmInfo( vmText::_($string,false).' Vendor Copy');
			vmTrace('notifyCustomer3');
		}
	...
	private function notifyCustomer($virtuemart_order_id, $newOrderData = 0 ) {
		...
		if($res!=-1){
			vmInfo( vmText::_($string,false).' CUSTOMER '.$order['details']['BT']->email);
		}
...
private function notifyCustomerver($virtuemart_order_id, $newOrderData = 0 ) {
		...
		// Send the email
	$db =& JFactory::getDBO();
		$sql = "SELECT `email` FROM #__agentbase_agents where `name`='#notify_verifiy'"; 
		$db->setQuery($sql);
		$verifiyz = $db->loadresult();
		
		$res = shopFunctionsF::renderMail('notf', $verifiyz, $vars, null,$vars['doVendor2'],$this->useDefaultEmailOrderStatus);
		...
		if($res!=-1){
			vmInfo( vmText::_($string,false).' verifiy '.$verifiyz);
		}
...
private function notifyCustomerag($virtuemart_order_id, $newOrderData = 0 ) {
		...
		// Send the email
$db =& JFactory::getDBO();
		$sql = "SELECT `virtuemart_user_id` FROM #__virtuemart_orders  where `virtuemart_order_id`= ".$virtuemart_order_id; 
		$db->setQuery($sql);
		$shoperid = $db->loadresult();
		$db2 =& JFactory::getDBO();
		$sql2 = "SELECT `cb_salesagentemail` FROM #__comprofiler  where `user_id`= ".$shoperid; 
		$db2->setQuery($sql2);
		$agentmail = $db2->loadresult();
		
	
		
		$res = shopFunctionsF::renderMail('notf', $agentmail, $vars, null,$vars['doVendor2'],$this->useDefaultEmailOrderStatus);
		...
		if($res!=-1){
			vmInfo( vmText::_($string,false).' AGENT '.$agentmail);
		}
...
private function notifyCustomersh($virtuemart_order_id, $newOrderData = 0 ) {
		...
		// Send the email
$db =& JFactory::getDBO();
		$sql = "SELECT `email` FROM #__agentbase_agents where `name`='#notify_shipping'"; 
		$db->setQuery($sql);
		$shippingz = $db->loadresult();
		
		//if (shopFunctionsF::renderMail('notf', $shippingz, $vars, null,$vars['doVendor2']
		
		$res = shopFunctionsF::renderMail('notf', $shippingz, $vars, null,$vars['doVendor2'],$this->useDefaultEmailOrderStatus);
		...
		if($res!=-1){
			vmInfo( vmText::_($string,false).' SHIPPING '.$shippingz);
		}
...
private function notifyCustomerart($virtuemart_order_id, $newOrderData = 0 ) {
		...
		// Send the email
$db =& JFactory::getDBO();
		$sql = "SELECT `email` FROM #__agentbase_agents where `name`='#notify_art'"; 
		$db->setQuery($sql);
		$artzzz = $db->loadresult();
		
		//if (shopFunctionsF::renderMail('notf', $artzzz, $vars, null,$vars['doVendor2']
		
		$res = shopFunctionsF::renderMail('notf', $artzzz, $vars, null,$vars['doVendor2'],$this->useDefaultEmailOrderStatus);
		...
		if($res!=-1){
			vmInfo( vmText::_($string,false).' ART '.$artzzz);
		}
...
private function notifyCustomerother($virtuemart_order_id, $newOrderData = 0,$othermailz ) {
		...
		$res = shopFunctionsF::renderMail('notf', $othermailz, $vars, null,$vars['doVendor2'],$this->useDefaultEmailOrderStatus);
		...
		if($res!=-1){
			vmInfo( vmText::_($string,false).' OTHER '.$othermailz);
		}
...
private function notifyCustomeoadmin($virtuemart_order_id, $newOrderData = 0 ) {
		...
		// Send the email
$db =& JFactory::getDBO();
		$sql = "SELECT `email` FROM #__agentbase_agents where `name`='#notify_office_admin'"; 
		$db->setQuery($sql);
		$artzzz = $db->loadresult();
		...
		if($res!=-1){
			vmInfo( vmText::_($string,false).' Office Admin '.$artzzz);
		}
...
private function notifyCustomerfax($virtuemart_order_id, $newOrderData = 0 ) {
		...
		// Send the email
$db =& JFactory::getDBO();
		$sql = "SELECT `email` FROM #__agentbase_agents where `name`='#notify_fax_notification'"; 
		$db->setQuery($sql);
		$artzzz = $db->loadresult();
		...
		if($res!=-1){
			vmInfo( vmText::_($string,false).' V FAX '.$artzzz);
		}
...
private function notifyCustomerengraver($virtuemart_order_id, $newOrderData = 0 ) {
...
if (isset($newOrderData['customer_notified']) && $newOrderData['customer_notified']==0) {
		...
		// Send the email
$db =& JFactory::getDBO();
		$sql = "SELECT `email` FROM #__agentbase_agents where `name`='#notify_engraver'"; 
		$db->setQuery($sql);
		$artzzz = $db->loadresult();
		...
		if($res!=-1){
			vmInfo( vmText::_($string,false).' Engraver '.$artzzz);
		}
...
private function notifyCustomerproduction($virtuemart_order_id, $newOrderData = 0 ) {
		...
		// Send the email
$db =& JFactory::getDBO();
		$sql = "SELECT `email` FROM #__agentbase_agents where `name`='#notify_production'"; 
		$db->setQuery($sql);
		$artzzz = $db->loadresult();
		...
		if($res!=-1){
			vmInfo( vmText::_($string,false).' Production '.$artzzz);
		}
...
private function notifyCustomerleader($virtuemart_order_id, $newOrderData = 0 ) {
